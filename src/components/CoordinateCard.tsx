...
const CoordinateCard: Component<{
      ...
      <li class="flex items-center col-span-1 overflow-hidden bg-white border border-gray-200 rounded-md shadow-sm">
        <div class="flex-1 px-4 py-2 truncate">
          <div class="text-sm font-medium leading-5 text-gray-900 transition duration-150 ease-in-out hover:text-gray-600">
            {Coordinate.fullFormatText(props.format)}
          </div>
          <p class={`text-sm leading-5 text-gray-500 ${!props.coordinate && "opacity-0"}`}>
            {props.coordinate?.toFormat(props.format, props.showTrailingZeros) || "N 24 24"}
          </p>
        </div>
        <div class="flex-shrink-0 pr-2">
          <button
            onClick={() =>
              props.coordinate && copyCoordinate(props.coordinate.toFormat(props.format, props.showTrailingZeros))
            }
